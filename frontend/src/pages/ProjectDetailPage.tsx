    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <button type="button" onClick={onCancel} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontSize: '1.2rem', lineHeight: 1, padding: 4 }}>✕</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 22 }}>
          ...
          <div className="field" style={{ marginBottom: 0 }}>
            <label>Remarks</label>
            <input type="text" value={remarks} onChange={(e) => setRemarks(e.target.value)} placeholder="Any update or note about this task…" />
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button type="button" className="btn btn-ghost" onClick={onCancel} disabled={saving}>Cancel</button>
          <button type="button" className="btn btn-primary" disabled={saving || !status} onClick={() => onSave({ status, remarks, plannedStart, plannedEnd })}>
...
function AddTaskModal({ open, projectId, onSaved, onCancel }: { open: boolean; projectId: string; onSaved: () => void; onCancel: () => void }) {
  ...
  useEffect(() => {
    if (open) { setTitle(''); setDepartment(''); setStatus('NOT_STARTED'); setPlannedStart(''); setPlannedEnd(''); setRemarks(''); }
  }, [open]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await api('/tasks', { method: 'POST', body: JSON.stringify({ projectId, title: title.trim(), department: department.trim(), status, plannedStart: plannedStart || undefined, plannedEnd: plannedEnd || undefined, remarks: remarks || undefined }) });
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(2px)' }}>
      <div style={{ background: 'var(--surface)', borderRadius: 12, padding: '1.75rem 2rem', maxWidth: 500, width: '92%', boxShadow: '0 20px 60px rgba(0,0,0,0.25)', border: '1px solid var(--line)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 22 }}>
          <div className="field" style={{ marginBottom: 0 }}>
            ...
            <input autoFocus placeholder="e.g. Foundation concrete work" value={title} onChange={(e) => setTitle(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleSave(); if (e.key === 'Escape') onCancel(); }} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div className="field" style={{ marginBottom: 0 }}>
          ...
          <div className="field" style={{ marginBottom: 0 }}>
            <label>Remarks</label>
            <input placeholder="Any notes about this task…" value={remarks} onChange={(e) => setRemarks(e.target.value)} />
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button type="button" className="btn btn-ghost" onClick={onCancel} disabled={saving}>Cancel</button>
          <button type="button" className="btn btn-primary" disabled={saving || !title.trim() || !department.trim()} onClick={handleSave}>
            {saving ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Saving…</> : <><Plus className="w-3.5 h-3.5" /> Add task</>}
          </button>
        </div>
      </div>
    </div>
...
// ── Tasks Tab ──────────────────────────────────────────────────────────────────
function TasksTab({ tasks, projectId, onRefresh }: { tasks: TaskRow[]; projectId: string; onRefresh: () => void }) {
  ...
  const handleEditSave = async (values: { status: string; remarks: string; plannedStart: string; plannedEnd: string }) => {
    ...
    setEditSaving(true);
    try {
      await api(`/tasks/${editTarget.id}`, { method: 'PATCH', body: JSON.stringify({ status: values.status, remarks: values.remarks || undefined, plannedStart: values.plannedStart || undefined, plannedEnd: values.plannedEnd || undefined }) });
  ...
  const iconBtnStyle: React.CSSProperties = { background: 'none', border: 'none', borderRadius: 5, color: 'var(--muted)', cursor: 'pointer', padding: '4px 5px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.12s, color 0.12s' };

  return (
    <>
      <AddTaskModal open={showAdd} projectId={projectId} onSaved={() => { setShowAdd(false); onRefresh(); }} onCancel={() => setShowAdd(false)} />
      <EditTaskModal task={editTarget} onSave={handleEditSave} onCancel={() => setEditTarget(null)} saving={editSaving} />
      <Card>
        <CardHeader title="Tasks" subtitle={`${tasks.length} task${tasks.length !== 1 ? 's' : ''}`} actions={<button type="button" className="btn btn-primary btn-sm" onClick={() => setShowAdd(true)}><Plus className="w-3.5 h-3.5" /> Add task</button>} />
        <CardBody className="p-0">
          <table className="tbl">
            <thead>
              <tr>
              ...
              {tasks.map((t) => (
                <tr key={t.id} className={t.isDelayed ? 'delayed' : ''}>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <button type="button" title="Edit task" onClick={() => setEditTarget(t)} style={iconBtnStyle}
                      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = 'var(--surface-2)'; (e.currentTarget as HTMLElement).style.color = 'var(--green)'; }}
                      onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = 'none'; (e.currentTarget as HTMLElement).style.color = 'var(--muted)'; }}>
                      <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M11.5 2.5a1.414 1.414 0 012 2L5 13H2v-3L11.5 2.5z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/></svg>
                    </button>
                  </td>
                </tr>
              ))}
              {tasks.length === 0 && (
                <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--muted)', padding: '2.5rem' }}>No tasks yet.{' '}<button type="button" className="btn btn-link btn-sm" style={{ display: 'inline', padding: 0 }} onClick={() => setShowAdd(true)}>Add the first task →</button></td></tr>
              )}
            </tbody>
          </table>
        </CardBody>
...
function DeleteConfirmModal({ item, onConfirm, onCancel, deleting }: { item: { id: string; description: string } | null; onConfirm: () => void; onCancel: () => void; deleting: boolean }) {
  if (!item) return null;
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(2px)' }}>
      <div style={{ background: 'var(--surface)', borderRadius: 12, padding: '1.75rem 2rem', maxWidth: 420, width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.25)', border: '1px solid var(--line)' }}>
        <div style={{ width: 44, height: 44, borderRadius: '50%', background: '#fee2e2', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
          <svg width="20" height="20" viewBox="0 0 16 16" fill="none"><path d="M2 4h12M5 4V2h6v2M6 7v5M10 7v5M3 4l1 9a1 1 0 001 1h6a1 1 0 001-1l1-9" stroke="#ef4444" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </div>
        <div style={{ fontWeight: 700, fontSize: '1rem', marginBottom: 6, color: 'var(--ink)' }}>Delete WBS line item?</div>
        <div style={{ fontSize: '0.875rem', color: 'var(--muted)', marginBottom: 20, lineHeight: 1.5 }}>
          You are about to permanently delete <strong style={{ color: 'var(--ink)' }}>"{item.description}"</strong>. This action cannot be undone.
        </div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button type="button" className="btn btn-ghost" onClick={onCancel} disabled={deleting}>Cancel</button>
          <button type="button" onClick={onConfirm} disabled={deleting} style={{ background: '#ef4444', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 18px', fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
            {deleting ? <><Loader2 style={{ width: 14, height: 14 }} className="animate-spin" /> Deleting…</> : 'Yes, delete it'}
          </button>
        </div>
      </div>
    </div>
...
function EditLineItemModal({ item, categoryOptions, onSave, onCancel, saving }: { item: { id: string; description: string; estimated: number; categoryName: string } | null; categoryOptions: string[]; onSave: (values: { description: string; estimated: number; categoryName: string }) => void; onCancel: () => void; saving: boolean }) {
  const [desc,      setDesc]      = useState('');
  const [estimated, setEstimated] = useState('');
  const [category,  setCategory]  = useState('');

  useEffect(() => {
    if (item) { setDesc(item.description); setEstimated(String(item.estimated)); setCategory(item.categoryName); }
  }, [item?.id]);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(2px)' }}>
      <div style={{ background: 'var(--surface)', borderRadius: 12, padding: '1.75rem 2rem', maxWidth: 460, width: '92%', boxShadow: '0 20px 60px rgba(0,0,0,0.25)', border: '1px solid var(--line)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
          <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--ink)' }}>Edit WBS line item</div>
          <button type="button" onClick={onCancel} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontSize: '1.2rem', lineHeight: 1 }}>✕</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="field" style={{ marginBottom: 0 }}>
            ...
            <input value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="e.g. PV Modules supply — 77 kW" />
          </div>
          <div className="field" style={{ marginBottom: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 22 }}>
          <button type="button" className="btn btn-ghost" onClick={onCancel} disabled={saving}>Cancel</button>
          <button type="button" className="btn btn-primary" disabled={saving || !desc.trim()} onClick={() => onSave({ description: desc.trim(), estimated: estimated ? Math.round(Number(estimated)) : 0, categoryName: category })}>
            {saving ? <><Loader2 style={{ width: 14, height: 14 }} className="animate-spin" /> Saving…</> : 'Save changes'}
          </button>
        </div>
      </div>
    </div>
...
function WbsTable({ wbs, projectId, onRefresh }: { wbs: unknown[]; projectId: string; onRefresh: () => void }) {
  ...
  const [editTarget, setEditTarget] = useState<{ id: string; description: string; estimated: number; categoryName: string } | null>(null);
  const [editSaving, setEditSaving] = useState(false);
  ...
  const saveEdit = async (values: { description: string; estimated: number; categoryName: string }) => {
    ...
    setEditSaving(true);
    try {
      await api(`/projects/${projectId}/wbs/line-items/${editTarget.id}`, { method: 'PATCH', body: JSON.stringify({ description: values.description, estimated: values.estimated, categoryName: values.categoryName || undefined }) });
  ...
  const SOLAR_CATEGORIES = ['Land', '33KV Transmission Line', 'Substation Work', 'Yard & Civil', 'Yard Products', 'Panel', 'MMS & Module Erection', 'DC & AC Cabling', 'Infrastructure', 'Liaisoning', 'Others'];
  ...
  const addLineItem = async () => {
    if (!form.categoryName || !form.description) { toast.error('Category and description are required'); return; }
    setSaving(true);
    try {
      await api(`/projects/${projectId}/wbs/line-items`, { method: 'POST', body: JSON.stringify({ categoryName: form.categoryName, description: form.description, estimated: form.estimated ? Math.round(parseFloat(form.estimated)) : 0 }) });
  ...
  return (
    <>
      <EditLineItemModal item={editTarget} categoryOptions={categoryOptions} onSave={saveEdit} onCancel={() => setEditTarget(null)} saving={editSaving} />
      <DeleteConfirmModal item={deleteTarget} onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} deleting={deleting} />
            ...
            <CardBody>
              <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                  ...
                  <button type="button" className="btn btn-ghost" onClick={() => setShowForm(true)}><Plus className="w-3.5 h-3.5" /> Add manually</button>
                </div>
                <p style={{ color: 'var(--muted)', fontSize: '0.78rem' }}>Solar EPC defaults include: Land · 33KV Line · Substation · Yard &amp; Civil · Panels · MMS · Cabling · Liaisoning · Others</p>
              </div>
            </CardBody>
          </Card>
        )}
            ...
            <CardBody>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 140px auto', gap: '0.5rem', alignItems: 'end' }}>
                ...
                <div className="field" style={{ marginBottom: 0 }}>
                  ...
                  <input placeholder="e.g. PV Modules supply — 77 kW" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
                </div>
                <div className="field" style={{ marginBottom: 0 }}>
                  <label style={{ fontSize: '0.75rem' }}>Estimated (₹)</label>
                  <input type="number" placeholder="0" value={form.estimated} onChange={(e) => setForm({ ...form, estimated: e.target.value })} />
                </div>
                <button type="button" className="btn btn-primary btn-sm" disabled={saving || !form.categoryName || !form.description} onClick={addLineItem} style={{ whiteSpace: 'nowrap' }}>
        ...
        {cats.length > 0 && (
          <Card>
            <CardBody className="p-0">
              ...
              >
                <colgroup>
                  {/* Line item — takes all remaining space */}
                  <col style={{ width: 'auto' }} />
                  {/* Four money columns — fixed, right-aligned */}
                  <col style={{ width: '11%' }} />
                  <col style={{ width: '11%' }} />
                  <col style={{ width: '11%' }} />
                  <col style={{ width: '11%' }} />
                ...
                <tbody>
                  {cats.map((cat) => (
                    <Fragment key={cat.name}>
                      {/* Category header row spans all 7 columns */}
                      ...
                      {cat.lineItems.map((li) => (
                        <tr key={li.id}>
                          ...
                          {/* Edit + Delete — single cell, no extra phantom column */}
                          <td style={{ textAlign: 'center', whiteSpace: 'nowrap', padding: '0 6px' }}>
                            ...
                            >
                              <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M2 4h12M5 4V2h6v2M6 7v5M10 7v5M3 4l1 9a1 1 0 001 1h6a1 1 0 001-1l1-9" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                            </button>
                          </td>
                        </tr>
                      ))}
