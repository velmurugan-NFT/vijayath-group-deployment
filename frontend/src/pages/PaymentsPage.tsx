...
export function PaymentsPage() {
  ...
  useEffect(() => {
    load();
    // Always fetch POs using the current project query so the dropdown
    // reflects the active project scope — never derive it from items.
    api<{ id: string; poNumber: string; status: string }[]>(`/pos${pq}`)
      .then((p) => setPos(p.filter((x) => x.status === 'APPROVED')));
  }, [pq]);

  // Reset the selected PO whenever the project scope changes so the
  // previously selected PO (from another project) doesn't persist.
  useEffect(() => {
    setPoId('');
  }, [activeProject?.id]);
    ...
    <div>
      ...
      {approvedId && (
        <div className="card card-pad mb-4">
          ...
          />
          <button type="button" className="btn-primary" onClick={execute}>
            Execute payment
          </button>
        </div>
      )}
