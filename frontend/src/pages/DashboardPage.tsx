...
export function DashboardPage() {
  ...
  // Re-fetch whenever activeProject changes so dashboard data is always scoped
  const load = useCallback(() => {
    setLoading(true);
    const qs = activeProject?.id ? `?projectId=${activeProject.id}` : '';
    api<Record<string, unknown>>(`/dashboard${qs}`)
      .then(setData)
      .catch(() => setData(null))
      .finally(() => setLoading(false));
  }, [activeProject?.id]); // ← key fix: re-run when project changes

  useEffect(() => {
    load();
  }, [load]); // load already captures activeProject.id via useCallback
  ...
  if (!data) {
    return (
      <p className="text-vijayanth-danger">
  ...
  // Use activeProject name directly — no need to filter the projects array
  const scopeLabel = activeProject?.name ?? 'Portfolio';
  const subtitle   = `${todayLabel()} · ${scopeLabel} · ${roleLabel}`;
        ...
        actions={
          <>
            {user?.role === 'PROJECT_HEAD' && (
              <>
                <Link to="/daily-status" className="btn btn-ghost">
                  <Upload className="w-3.5 h-3.5" /> Daily status
                </Link>
