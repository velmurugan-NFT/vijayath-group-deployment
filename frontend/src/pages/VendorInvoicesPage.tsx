...
type Inv = { id: string; invoiceNumber: string; amount: number; invoiceDate: string; po?: { poNumber: string; project?: { id: string; name: string }; vendor?: { name: string } } };
...
export function VendorInvoicesPage() {
  ...
  const [form, setForm] = useState({ poId: '', invoiceNumber: '', amount: 0, invoiceDate: new Date().toISOString().slice(0, 10) });

  const load = () => api<Inv[]>('/vendor-invoices').then(setItems);
  useEffect(() => {
    load();
  ...
  const submit = async () => {
    await api('/vendor-invoices', { method: 'POST', body: JSON.stringify(form) });
  ...
  return (
    <div>
      <PageHeader title="Vendor invoices" subtitle="Invoices linked to POs" actions={<Button onClick={() => setOpen(true)}>Add invoice</Button>} />
      <DataTable
        columns={[
          ...
          { key: 'proj', header: 'Project', render: (r) => r.po?.project ? <Link to={`/projects/${r.po.project.id}`} className="underline">{r.po.project.name}</Link> : '—' },
        ]}
        data={items}
        keyFn={(r) => r.id}
      />
      <FormDialog open={open} onOpenChange={setOpen} title="Vendor invoice" onSubmit={submit}>
        ...
        <div><Label>Date</Label><Input type="date" value={form.invoiceDate} onChange={(e) => setForm({ ...form, invoiceDate: e.target.value })} /></div>
      </FormDialog>
    </div>
  );
}
