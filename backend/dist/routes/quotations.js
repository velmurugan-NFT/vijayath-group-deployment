...
router.post('/:id/quotes', requireAuth, async (req, res, next) => {
    try {
        ...
        const notesValue = (gstPct != null || paymentTerms || unitPrice != null || quantity != null)
            ? JSON.stringify({
                gstPct: gstPct ?? null,
...
router.patch('/:id/quotes/:quoteId', requireAuth, async (req, res, next) => {
    try {
        const qr = await prisma.quotationRequest.findUnique({ where: { id: req.params.id } });
        if (!qr) {
        ...
        const notesValue = (gstPct != null || paymentTerms != null)
            ? JSON.stringify({
                _meta: true,
                gstPct: gstPct ?? null,
                paymentTerms: paymentTerms ?? null,
...
router.delete('/:id/quotes/:quoteId', requireAuth, async (req, res, next) => {
    try {
        const qr = await prisma.quotationRequest.findUnique({ where: { id: req.params.id } });
        if (!qr) {
            res.status(404).json({ error: 'QR not found' });
...
router.post('/:id/select-winner', requireAuth, async (req, res, next) => {
    try {
