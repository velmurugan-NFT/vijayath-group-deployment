...
export async function getThreshold(role) {
    const t = await prisma.approvalThreshold.findUnique({ where: { role } });
    const n = (v, fallback) => Number(v ?? BigInt(fallback));
    if (role === Role.PROJECT_HEAD)
        return n(t?.ceiling, 50000);
    if (role === Role.SECTOR_HEAD)
        return n(t?.ceiling, 500000);
    return n(t?.ceiling, Number.MAX_SAFE_INTEGER);
}
export function resolveRequiredApproverRole(amount) {
    if (amount <= 50000)
...
export function canRoleApprove(role, amount) {
