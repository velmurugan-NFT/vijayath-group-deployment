...
router.get('/pl', requireAuth, async (req: AuthRequest, res, next) => {
  try {
...
router.get('/budget-variance', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    assertCan(req.user!, 'reports:export');
    const pf = await projectFilter(req.user!);
...
router.get('/receivables-ageing', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    assertCan(req.user!, 'reports:export');
    const pf = await projectFilter(req.user!);
    const projects = await prisma.project.findMany({
