...
router.get('/pl', requireAuth, async (req: AuthRequest, res) => {
...
router.get('/budget-variance', requireAuth, async (req: AuthRequest, res) => {
  assertCan(req.user!, 'reports:export');
  const pf = await projectFilter(req.user!);
...
router.get('/receivables-ageing', requireAuth, async (req: AuthRequest, res) => {
  assertCan(req.user!, 'reports:export');
  const pf = await projectFilter(req.user!);
  const projects = await prisma.project.findMany({
    where: { ...pf, parentId: { not: null } },
